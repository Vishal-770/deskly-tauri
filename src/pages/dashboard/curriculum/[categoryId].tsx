...
export default function CategoryCoursesPage() {
  ...
  return shell(
    <div className="w-full space-y-5 px-2 py-4 font-saira relative">
      ...
      {/* ── Course List ─────────────────────────────────────────────────────── */}
      {filtered.length === 0 ? (
        <div className="relative z-10 flex flex-col items-center justify-center py-20 gap-3 text-center bg-muted/15 dark:bg-muted/15 dark:bg-[#0e0e0f]/20 border border-border/40 dark:border-border/10 rounded-md">
        ...
        <div className="relative z-10 flex flex-col gap-3">
          {filtered.map((course) => {
            ...
            return (
              <div
                key={course.code}
                className="p-4 bg-card/80 border border-border/40 rounded-lg shadow-sm flex items-start justify-between gap-4 backdrop-blur-md"
              >
